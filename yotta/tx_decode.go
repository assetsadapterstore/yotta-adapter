...
func (decoder *TransactionDecoder) SubmitRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) (*openwallet.Transaction, error) {
	...
	stxJSON, _ := json.Marshal(&stx)
	decoder.wm.Log.Debugf("stxjson: %s", string(stxJSON))
...
func (decoder *TransactionDecoder) createRawTransaction(
	...
	action := NewTransfer(accountResp.AccountName, to, quantity, memo)
...
func NewTransfer(from, to eos.AccountName, quantity eos.Asset, memo string) *eos.Action {
	return &eos.Action{
		Account: eos.AN("ysr.ystar"),
		Name:    eos.ActN("yrctransfer"),
